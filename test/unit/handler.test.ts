...
function handler(extra: Record<string, unknown> = {}) {
  return uploadHandler({
    ...
    routes: {
      ...
      large: { constraints: { maxSize: '2gb', contentTypes: null }, onBeforeUpload: () => ({ path: 'large/1.bin' }) },
      avatar: { constraints: { maxSize: '2mb' }, onBeforeUpload: () => ({ path: 'avatar/demo' }) },
    },
    ...extra,
  });
}
...
describe('defaults', () => {
  ...
  test('the key order of routes changes nothing', async () => {
    const first = uploadHandler({
      bucket: bucket(),
      constraints: { maxSize: '20mb' },
      onBeforeUpload: ({ route }) => ({ path: `${route}/1` }),
      routes: { alpha: {}, omega: { constraints: { maxSize: '1kb' } } },
    });
