...
// The proxied-upload primitive under useUploadProxy, driven against SPEC section 1's routes with
// the bytes really landing through put().
const restore: (() => void)[] = [];
beforeAll(async () => {
...
describe('useUploadProxy primitive', () => {
  test('start({file}) is multipart under field "file"; the route answers JSON', async () => {
    const png = new Uint8Array(new ArrayBuffer(900));
    png.set(PNG);
  ...
  test('start({body: file}) is the raw body for a streaming route', async () => {
    const png = new Uint8Array(new ArrayBuffer(950));
    png.set(PNG);
    const file = new File([png], 'me.png', { type: 'image/png' });
